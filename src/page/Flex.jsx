...
const Flex = () => {
  ...
  useEffect(() => {
    const fetchFlexImages = async () => {
      try {
        setLoading(true);
        const snapshot = await getDocs(collection(db, "uploads"));
        const items = snapshot.docs
          .map((doc) => ({ id: doc.id, ...doc.data() }))
          .filter((item) => item.category === "Flex");

        setFlexItems(items);
        setError(null);
      } catch (err) {
        console.error("Error fetching flex prints:", err);
  ...
  const allImages = flexItems.flatMap((item) =>
    item.imageUrls.map((url, idx) => ({
      id: `${item.id}-${idx}`,
      url,
      title: item.title || "Flex Banner Design",
      description: item.description || "Premium flex banner design",
      category: item.category || "Flex"
    }))
  );
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
          ...
          >
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-cyan-300 via-blue-300 to-teal-200">
              High-Impact Flex Banners
            </span>
            <br />
            <span className="text-white">For Maximum Visibility</span>
          </motion.h1>
          ...
          >
            Transform your marketing with durable, weather-resistant flex banners that command attention. Perfect for outdoor advertising, events, and promotions.
          </motion.p>
      ...
      >
        <div className="text-center mb-16">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            High-quality flex prints with vibrant colors and durable materials
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-xl shadow-sm max-w-md mx-auto p-8">
            ...
            <p className="text-gray-600 mb-6">We couldn't find any flex banner designs at the moment</p>
            <button
              onClick={() => window.location.reload()}
              className="px-6 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors"
            >
      ...
      <div className="bg-gradient-to-br from-[#0a192f] to-[#1a759f] py-16 px-4 sm:px-6 lg:px-8 text-center">
        <div className="max-w-4xl mx-auto">
          <h3 className="text-3xl font-bold text-white mb-4">Ready for Professional Flex Banners?</h3>
          <p className="text-xl text-white/90 mb-8">
            Let us create durable, eye-catching banners that get your message noticed
          </p>
          <motion.button
            whileHover={{
              scale: 1.05,
